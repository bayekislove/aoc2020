...
class Board
{
    ...
    public:
        ...
        int treesEncountered( int rowDiff, int colDiff )
        {
            int trees(0);
            int last_column(0);
            for( int i(colDiff); i < height; )
            {
...
{
    uint64_t res = 1;
    res *= get_entries().treesEncountered(3, 1);
    res *= get_entries().treesEncountered(1, 1);
    res *= get_entries().treesEncountered(5, 1);
    res *= get_entries().treesEncountered(7, 1);
    res *= get_entries().treesEncountered(1, 2);
